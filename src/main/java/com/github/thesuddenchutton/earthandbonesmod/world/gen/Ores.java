...
public class Ores{
	...
	public static void generateOres(final BiomeLoadingEvent e) {
		...
		generateOre2(e.getGeneration(), new BlockMatchTest(Blocks.STONE), Registration.SPIKETRAP.get().defaultBlockState(), 1, 0, 100, 400);
	}
	private static void generateOre(BiomeGenerationSettingsBuilder settings, RuleTest fillerType, BlockState state,
	          int veinSize, int minHeight, int maxHeight, int amount) {
				settings.addFeature(Decoration.UNDERGROUND_ORES,
						Feature.ORE.configured(new OreConfiguration(fillerType, state, veinSize)).range(new RangeDecoratorConfiguration(UniformHeight.of(VerticalAnchor.aboveBottom(minHeight), VerticalAnchor.aboveBottom(maxHeight)))).squared().count(amount));
		}
	private static void generateOre2(BiomeGenerationSettingsBuilder settings, RuleTest fillerType, BlockState state,
	          int veinSize, int minHeight, int maxHeight, int amount) {
				settings.addFeature(Decoration.UNDERGROUND_ORES,
						Feature.ORE.configured(new OreConfiguration(fillerType, state, veinSize)).range(new RangeDecoratorConfiguration(UniformHeight.of(VerticalAnchor.aboveBottom(minHeight), VerticalAnchor.aboveBottom(maxHeight)))).squared().count(amount));
	}
